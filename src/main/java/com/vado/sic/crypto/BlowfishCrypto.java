...
public class BlowfishCrypto {
    ...
    private static List<String> iteration(byte[] array) {
        ...
        try {
            ...
            Key key = new SecretKeySpec(generateKeyBytes(), ALGORITHM);
    ...
    private static byte[] encrypt(byte[] info, Key key) throws Exception {
    ...
    private static byte[] encryptTime(byte[] originalMessage, Key key) throws Exception {
        return encrypt(originalMessage, key);
    }

    private static byte[] decryptTime(byte[] encryptedBytes, Key key) throws Exception {
