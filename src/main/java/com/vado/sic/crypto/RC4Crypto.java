...
public class RC4Crypto {
    ...
    private static List<String> iteration(byte[] array) {
        ...
        try {
            ...
            byte[] encryptedBytes = encryptTime(array, key);
            ...
            byte[] decryptedBytes = decryptTime(encryptedBytes, key);
    ...
    private static byte[] encrypt(byte[] info, SecretKeySpec key) throws Exception {
        Cipher cipher = Cipher.getInstance(ALGORITHM);
        cipher.init(Cipher.ENCRYPT_MODE, key);
        return cipher.doFinal(info);
    }

    private static byte[] decrypt(byte[] cipherText, SecretKeySpec key) throws Exception {
        Cipher cipher = Cipher.getInstance(ALGORITHM);
        cipher.init(Cipher.DECRYPT_MODE, key);
        return cipher.doFinal(cipherText);
    }

    private static byte[] encryptTime(byte[] originalMessage, SecretKeySpec key) throws Exception {
        return encrypt(originalMessage, key);
    }

    private static byte[] decryptTime(byte[] encryptedBytes, SecretKeySpec key) throws Exception {
